...
// a struct
// something to encapsulate the string and the type of word
// that gets filled in
...
// array assignment with no specific limits
...
func main() {
	// Note that any of the functions called from the packages imported
	// has to start with a Capital letter
	// this is because of the rule that only functions starting with a
	// capital letter are 'exported' so you can call them here from another
	// package. It's not really the same as public and private but more like
	// direct and indirect reference or use
